...
import static io.mybear.common.constants.config.TrackerGlobal.*;
...
public class TrackerFunc {
    ...
    public static int trackerLoadFromConfFile(String fileName, String bindAddr) {
        ...
        FdfsGlobal.g_server_port = iniReader.getIntValue("port", FDFS_TRACKER_SERVER_DEF_PORT);
        if (FdfsGlobal.g_server_port <= 0) {
            FdfsGlobal.g_server_port = FDFS_TRACKER_SERVER_DEF_PORT;
        }
        ...
        TrackerGlobal.ggroups.setStoreServer((byte) iniReader.getIntValue("store_server", FDFS_STORE_SERVER_ROUND_ROBIN));
        if (!(TrackerGlobal.ggroups.getStoreServer() == FDFS_STORE_SERVER_FIRST_BY_IP
                || TrackerGlobal.ggroups.getStoreServer() == FDFS_STORE_SERVER_FIRST_BY_PRI
                || TrackerGlobal.ggroups.getStoreServer() == FDFS_STORE_SERVER_ROUND_ROBIN)) {
            log.warn(CommonConstant.LOG_FORMAT, "trackerLoadFromConfFile",
                    JSON.toJSON(iniReader), "store_server 's value %d is invalid, set to %d (round robin)", TrackerGlobal.ggroups.getStoreServer(), FDFS_STORE_SERVER_ROUND_ROBIN);
            TrackerGlobal.ggroups.setStoreServer(FDFS_STORE_SERVER_ROUND_ROBIN);
        }

        TrackerGlobal.ggroups.setDownloadServer((byte) iniReader.getIntValue("download_server", FDFS_STORE_SERVER_ROUND_ROBIN));
        if (!(TrackerGlobal.ggroups.getDownloadServer() == FDFS_DOWNLOAD_SERVER_ROUND_ROBIN
                || TrackerGlobal.ggroups.getDownloadServer() == FDFS_DOWNLOAD_SERVER_SOURCE_FIRST)) {
            log.warn(CommonConstant.LOG_FORMAT, "trackerLoadFromConfFile",
                    JSON.toJSON(iniReader), "download_server 's value %d is invalid, set to %d (round robin)", TrackerGlobal.ggroups.getDownloadServer(), FDFS_DOWNLOAD_SERVER_ROUND_ROBIN);
            TrackerGlobal.ggroups.setDownloadServer(FDFS_DOWNLOAD_SERVER_ROUND_ROBIN);
        }
        ...
        TrackerGlobal.ggroups.setStorePath((byte) iniReader.getIntValue("store_path", FDFS_STORE_PATH_ROUND_ROBIN));
        if (!(TrackerGlobal.ggroups.getStorePath() == FDFS_STORE_PATH_ROUND_ROBIN
                || TrackerGlobal.ggroups.getStorePath() == FDFS_STORE_PATH_LOAD_BALANC)) {
            log.warn(CommonConstant.LOG_FORMAT, "trackerLoadFromConfFile",
                    JSON.toJSON(iniReader), "store_path 's value %d is invalid, set to %d (round robin)", TrackerGlobal.ggroups.getStorePath(), FDFS_STORE_PATH_ROUND_ROBIN);
            TrackerGlobal.ggroups.setDownloadServer(FDFS_DOWNLOAD_SERVER_ROUND_ROBIN);
        }
...
//        }
        ...
        TrackerGlobal.g_max_connections = iniReader.getIntValue("max_connections", DEFAULT_MAX_CONNECTONS);
        if (TrackerGlobal.g_max_connections <= 0) {
            TrackerGlobal.g_max_connections = DEFAULT_MAX_CONNECTONS;
        }
        ...
        TrackerGlobal.g_work_threads = iniReader.getIntValue("work_threads", DEFAULT_WORK_THREADS);
...
//        }
        ...
        TrackerGlobal.g_sync_log_buff_interval =
                iniReader.getIntValue("sync_log_buffer_interval", SYNC_LOG_BUFF_DEF_INTERVAL);
        if (TrackerGlobal.g_sync_log_buff_interval <= 0) {
            TrackerGlobal.g_sync_log_buff_interval = SYNC_LOG_BUFF_DEF_INTERVAL;
        }

        TrackerGlobal.g_check_active_interval =
                iniReader.getIntValue("check_active_interval", CHECK_ACTIVE_DEF_INTERVAL);
        if (TrackerGlobal.g_check_active_interval <= 0) {
            TrackerGlobal.g_check_active_interval = CHECK_ACTIVE_DEF_INTERVAL;
        }
        ...
        TrackerGlobal.g_storage_sync_file_max_delay = iniReader.
                getIntValue("storage_sync_file_max_delay", DEFAULT_STORAGE_SYNC_FILE_MAX_DELAY);
        if (TrackerGlobal.g_storage_sync_file_max_delay <= 0) {
            TrackerGlobal.g_storage_sync_file_max_delay = DEFAULT_STORAGE_SYNC_FILE_MAX_DELAY;
        }

        TrackerGlobal.g_storage_sync_file_max_time = iniReader.getIntValue("storage_sync_file_max_time", DEFAULT_STORAGE_SYNC_FILE_MAX_TIME);
        if (TrackerGlobal.g_storage_sync_file_max_time <= 0) {
            TrackerGlobal.g_storage_sync_file_max_time = DEFAULT_STORAGE_SYNC_FILE_MAX_TIME;
        }
...
//        }
    ...
    static int trackerLoadStoreLookup(String fileName, IniFileReader iniReader) {
        String pGroupName = "";
        ggroups.setStoreLookup((byte) iniReader.getIntValue("store_lookup", FDFS_STORE_LOOKUP_ROUND_ROBIN));
        if (ggroups.getStoreLookup() == FDFS_STORE_LOOKUP_ROUND_ROBIN) {
            ggroups.setStoreLookup((byte) 0);
            return 0;
        }

        if (ggroups.getStoreLookup() == FDFS_STORE_LOOKUP_LOAD_BALANCE) {
            ggroups.setStoreLookup((byte) 0);
            return 0;
        }

        if (ggroups.getStoreLookup() != FDFS_STORE_LOOKUP_SPEC_GROUP) {
