...
public class TrunkShared {
    ...
    private static int SPLIT_FILENAME_BODY(char[] logic_filename, int filename_len, char[] true_filename, boolean check_path_index) {
        ...
        if (filename_len <= TrackerTypes.FDFS_LOGIC_FILE_PATH_LEN) {
            log.error(CommonConstant.LOG_FORMAT, "SPLIT_FILENAME_BODY", "", String.format("filename_len: %d is invalid, <= %d", filename_len, TrackerTypes.FDFS_LOGIC_FILE_PATH_LEN));
            return -1;
        }
        if (logic_filename[0] != TrackerTypes.FDFS_STORAGE_STORE_PATH_PREFIX_CHAR) { /* version < V1.12 */
    ...
    private static File trunk_file_do_lstat_func_ex(FdfsStorePaths pStorePaths, int store_path_index, char[] true_filename,
        ...
        if (filename_len != TrackerTypes.FDFS_TRUNK_FILENAME_LENGTH) {
            if (fileName.length() > CommonDefine.MAX_PATH_SIZE) {
                flag = CommonDefine.MAX_PATH_SIZE;
                System.arraycopy(fileName.toCharArray(), 0, full_filename, 0, CommonDefine.MAX_PATH_SIZE);
            } else {
        ...
        System.arraycopy(true_filename, TrackerTypes.FDFS_TRUE_FILE_PATH_LEN + TrackerTypes.FDFS_FILENAME_BASE64_LENGTH,
                src, 0, true_filename.length - TrackerTypes.FDFS_TRUE_FILE_PATH_LEN - TrackerTypes.FDFS_FILENAME_BASE64_LENGTH);
        trunk_file_info_decode(src, pTrunkInfo.getFile());
        pTrunkHeader.fileSize = TrackerTypes.FDFS_TRUNK_FILE_TRUE_SIZE(file_size);
    ...
    public static char[] trunk_get_full_filename_ex(FdfsStorePaths pStorePaths, FdfsTrunkFullInfo pTrunkInfo,
       ...
       String short_filename;
        ...
        String fullFileName = String.format("%s/data/" + TrackerTypes.FDFS_STORAGE_DATA_DIR_FORMAT + "/" + TrackerTypes.FDFS_STORAGE_DATA_DIR_FORMAT + "/%s",
    ...
    private static void trunk_file_info_decode(final char[] src, FdfsTrunkFileInfo pTrunkFile) {
        char[] buff = new char[TrackerTypes.FDFS_TRUNK_FILE_INFO_LEN];
        buff = Base64.base64_decode_auto(TrunkShared.base64Context, src, TrackerTypes.FDFS_TRUNK_FILE_INFO_LEN, buff);
