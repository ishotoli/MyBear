...
public class HashUtil {
    ...
    static int time33(ByteBuffer str, int hash) {
        int pos = str.position();
        int len = str.limit();
        for (int i = pos; i < len; i++)
            // (hash << 5) + hash 相当于 hash * 33
            hash = (hash << 5) + hash + (int) str.get(i);
        return hash;
    }

    public static int[] initHashCodes4() {
        int[] hash_codes = new int[4];
        final int CRC32_XINIT = 0xFFFFFFFF;
    ...
    public static void finishHashCodes4(int[] hash_codes) {
        final int CRC32_XOROT = 0xFFFFFFFF;
        hash_codes[0] = hash_codes[0] ^ CRC32_XOROT;
    }


    public static int crc32Hash(ByteBuffer key, int hash) {
        int n = key.limit();
        for (int i = key.position(); i < n; ++i)
            hash = (hash >> 8) ^ crctab[(hash & 0xff) ^ key.get(i)];
        return hash;
    }

    public static int additiveHash(ByteBuffer key) {
        int n = key.limit();
        int hash = n;
        for (int i = key.position(); i < n; i++)
            hash += key.get(i);
    ...
    public static int rotatingHash(ByteBuffer key) {
        int n = key.limit();
        int hash = n;
        for (int i = key.position(); i < n; i++)
            hash = (hash << 4) ^ (hash >> 28) ^ key.get(i);
        return (hash & 0x7FFFFFFF);
    }

    public static int simpleHash(ByteBuffer key, int h) {
        int p = key.position();
        int limit = key.limit();
        for (; p < limit; p++) {
            h = 31 * h + key.get(p);
    ...
    public static void hashCODES4(ByteBuffer key, int[] h) {
        int pos = key.position();
        int limit = key.limit();
        h[0] = crc32Hash(key, h[0]);
        key.position(pos).limit(limit);
        h[1] = ELFhash(key, h[1]);
        key.position(pos).limit(limit);
        h[2] = simpleHash(key, h[2]);
        key.position(pos).limit(limit);
        h[3] = time33(key, h[1]);
    }
     ...
     * @return
     */
    public static int BKDRHash(ByteBuffer key, int prime) {
        int hash = 0;
        int n = key.limit();
        for (int i = key.position(); i < n; ++i)
            hash = prime * hash + key.get(i);
        return (hash & 0x7FFFFFFF);
    }
    ...
    public static int ELFhash(ByteBuffer key, int h) {
        int n = key.limit();
        for (int i = key.position(); i < n; i++) {
            h = (h << 4) + key.get(i);
